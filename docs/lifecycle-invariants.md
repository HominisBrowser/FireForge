...
2. **At most one real engine/patch mutation runs at a time** per resource:
   ...
   writes) on a per-directory lock. Dry-runs skip the engine-session lock —
   they only read.
