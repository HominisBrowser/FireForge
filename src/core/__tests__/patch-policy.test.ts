...
describe('patch policy evaluation', () => {
  ...
  it('reports duplicate numeric orders in policy-owned queues', () => {
    const issues = evaluatePatchPolicy(
      policyConfig({ requireDescription: false }),
      manifest([
        patch({ filename: '200-ui-a.patch', order: 200, name: 'a' }),
        patch({ filename: '200-ui-b.patch', order: 200, name: 'b' }),
      ])
    );

    expect(issues.map((issue) => issue.code)).toContain('order-collision');
    expect(issues.find((issue) => issue.code === 'order-collision')?.message).toContain(
      'order 200'
    );
  });
