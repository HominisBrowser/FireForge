 ...
 * refused up front if their renumber would touch a reserved range, giving
 * one error for the whole shift instead of per-patch policy findings. The
...
export async function projectPlacementForLint(
  ...
  // pre-existing breakage rather than a consequence of this placement, and
  // calling it one would misattribute. Entries are already in memory,
    ...
    try {
      ...
      if (input.onCommitted) {
        try {
          ...
          // re-throw, which would look like the export itself failed.
          warn(
