 ...
 * extracted content. For a binary file there is nothing to extract, since
 * the payload is base85, so before 0.45.0 the recovery threw and the import
...
describe('binary new-file conflict recovery (real git)', () => {
   ...
   * the binary file, and leaves a different file already at that path. That
   * is the exact conflict shape `applySinglePatch` recovers from.
   */
  ...
  it('resolves the conflict when a DIFFERENT file already occupies the path', async () => {
    ...
    expect(summary.succeeded).toHaveLength(1);
    // The patch's bytes won byte-for-byte, rather than a utf-8 round-trip
    // of them.
    expect(await readFile(join(engine, FONT))).toEqual(PATCH_BYTES);
  });
});
