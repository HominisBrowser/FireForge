...
export async function buildPatchQueueContext(
  ...
  for (const patch of patches) {
    ...
    // files. Paths in newFilePaths are skipped on purpose: they are
...
// `patch-lint-creators.ts` with its per-context memoisation. It is
 ...
 * - `import "specifier"`            (side-effect imports, where the `from`
...
export function extractImportSpecifiersWithLines(source: string): ExtractedSpecifier[] {
  ...
  // ChromeUtils.importESModule("resource://...") is a Firefox single-module import
 ...
 *
 * Conservative by design: `resource://` URLs are not resolved to engine
 ...
 *
 * Keyed on `newFiles`: this index answers "which patch creates
 ...
 * file's added lines only, so it flags imports *this patch introduces*, not
...
function eachForwardImportSite(
  ...
  const checkSite = (
    ...
    if (includeBareGetterLines) {
      ...
      // property line (found here), so report each site once.
...
export function lintPatchQueueForwardImports(ctx: PatchQueueContext): PatchLintIssue[] {
  ...
  eachForwardImportSite(ctx, newFileIndex, (entry, sitePath, specifier, cleaned, laterOwners) => {
    ...
    const fingerprintOwners = [...laterOwners]
      ...
      .join(',');
    // Lowest ordinal that lands after every later-ordered creator. It
    ...
    // `patch reorder` would refuse it, so suppress the hint and point at
