 ...
 * This module is that report. It deliberately reuses the banner and
 ...
 * a real state in a hand-edited file, and one the report must show rather
 * than silently drop.
 */
 ...
 *
 * Deliberately not restricted to the base `:root`: the question `token show`
 * answers is "what does this token resolve to", and a token whose dark
 * override differs from its base value is precisely the case worth seeing.
 *
 * @param lines - Tokens CSS split into lines
...
export function collectTokenBlockValues(lines: string[], tokenName: string): TokenBlockValue[] {
  ...
  for (let i = 0; i < masked.length; i++) {
    ...
    // precedes the `{`; an empty selector (a continuation line) still needs
    // a stack entry or the closing `}` pops the wrong one.
