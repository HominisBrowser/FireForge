...
/** A single-chunk archive Response (bodies are single-use, so build one per call). */
...
describe('checksum-based cache validation', () => {
  it('invalidates cache when sha256 does not match', async () => {
    ...
    // Mock createReadStream for sha256File. It returns data with a different
    ...
    // Now the fetch for re-download: provide a fresh body each call
