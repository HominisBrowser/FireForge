...
describe('downloadCommand integration', () => {
  ...
  it('keeps stable and ESR cache entries separate', async () => {
    ...
    const esrArchive = await makeTarXzArchive(projectRoot, 'esr.tar.xz', 'firefox-146.0esr', {
      'browser/config/version.txt': '146.0esr\n',
    });
    ...
    const esrCache = join(
      projectRoot,
      '.fireforge/cache',
      getTarballFilename('146.0esr', 'firefox-esr')
    );
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(
      2,
      getDownloadUrl('146.0esr', 'firefox-esr'),
      // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment -- vitest asymmetric matcher
      expect.objectContaining({ signal: expect.any(AbortSignal) })
    );
  });
  ...
  it('replaces stale partial downloads atomically', async () => {
    const archivePath = await makeTarXzArchive(projectRoot, 'esr.tar.xz', 'firefox-146.0esr', {
      'browser/config/version.txt': '146.0esr\n',
    });
    ...
    const cacheFile = join(
      projectRoot,
      '.fireforge/cache',
      `${getTarballFilename('146.0esr', 'firefox-esr')}.part`
    );
    await writeFiles(projectRoot, {
      [join('.fireforge/cache', `${getTarballFilename('146.0esr', 'firefox-esr')}.part`)]:
        'partial',
    });
    ...
    await expect(
      readText(projectRoot, '.fireforge/cache/firefox-firefox-esr-146.0esr.source.tar.xz.json')
    ).resolves.toContain('"archiveVersion": "146.0esr"');
  ...
  it('invalidates corrupted cached archives after extraction failure and recovers on retry', async () => {
    ...
    const tarballName = getTarballFilename('146.0esr', 'firefox-esr');
    await writeFiles(projectRoot, {
      [join('.fireforge/cache', tarballName)]: 'not a real tarball',
      [join('.fireforge/cache', `${tarballName}.json`)]: JSON.stringify(
        {
          requestedVersion: '146.0esr',
          product: 'firefox-esr',
          archiveVersion: '146.0esr',
          url: getDownloadUrl('146.0esr', 'firefox-esr'),
    ...
    const archivePath = await makeTarXzArchive(projectRoot, 'retry.tar.xz', 'firefox-146.0esr', {
      'browser/config/version.txt': '146.0esr\n',
    });
    const archiveBody = await readFile(archivePath);
    fetchMock.mockResolvedValue(
    ...
    expect(versionFile).toBe('146.0esr\n');
