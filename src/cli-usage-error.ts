 ...
 * exit 1. That is an informational invocation, not a wrong flag — `patch`,
 * `token` and `furnace` already answer it with help and exit 0 through an
 * explicit action, and the groups without one must not exit 8 (or leak
 * commander's `(outputHelp)` placeholder into a `--json` envelope) merely
 * for lacking it. A genuine usage error has already had its message written
 * to stderr by commander; it becomes exit 8, and under `--json` additionally
 * gets the standard refusal envelope on stdout so a scripted consumer sees a
 * parseable failure.
 *
