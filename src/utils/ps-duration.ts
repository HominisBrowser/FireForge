 ...
 * wall clock). One parser handles both: a second hand-rolled copy is how
 * the two would drift, and the darwin dialect is the half that is easy to
 * get wrong.
 *
 * Dialects:
...
export function parsePsDuration(value: string): number {
  ...
  if (parts.length === 3) {
    // hh:mm:ss on linux. Seconds may not carry fractions but tolerate them.
    const [h, m, s] = parts.map(Number);
    if ([h, m, s].some((n) => Number.isNaN(n))) return NaN;
    return days * 86400 + (h ?? 0) * 3600 + (m ?? 0) * 60 + (s ?? 0);
  }
  if (parts.length === 2) {
    // mm:ss.cc on darwin. The minutes field accumulates without wrapping.
