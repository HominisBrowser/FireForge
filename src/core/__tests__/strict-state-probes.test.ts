 ...
 * project — silently returning false used to make commands proceed as if
 * no state existed (e.g. "0 patches", "not a git repository").
 *
 * chmod-based EACCES cannot be produced on Windows or as root, so the
 * whole suite skips there; the Ubuntu/macOS CI runs cover it.
 */
