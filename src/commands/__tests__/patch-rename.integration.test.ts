...
describe('patch rename', () => {
  ...
  it('renames the .patch file on disk and updates manifest filename + name', async () => {
    await seed(patchesDir, [makeMetadata('0044-ui-hominis-dock-widgets.patch', 44, ['a.js'])]);

    await patchRenameCommand(projectRoot, '0044-ui-hominis-dock-widgets.patch', {
      to: 'hominis-dock-primitive-widgets',
      yes: true,
    });

    expect(await fileExists(join(patchesDir, '0044-ui-hominis-dock-widgets.patch'))).toBe(false);
    expect(await fileExists(join(patchesDir, '0044-ui-hominis-dock-primitive-widgets.patch'))).toBe(
      true
    );

    const manifest = await loadManifest(patchesDir);
    expect(manifest.patches[0]?.filename).toBe('0044-ui-hominis-dock-primitive-widgets.patch');
    expect(manifest.patches[0]?.name).toBe('hominis-dock-primitive-widgets');
  });
