...
const MODULE_THRESHOLDS = {
  ...
  // Identity must degrade to the plain semver and never throw. The guarded
  // fallbacks are what the module exists for.
  ...
  // Pure pattern-based error-hint translator, so it is trivial to test.
  'src/core/mach-error-hints.ts': { lines: 98, branches: 95, functions: 100 },
  // Post-build audit (warn-only). It matters because misdetections here
  ...
  // decision must stay exhaustively covered, because a misclassified plan
  // reverts a patch-backed file past its owning patch.
  'src/core/discard-baseline.ts': { lines: 90, branches: 80, functions: 90 },
  // Per-patch moz.build sorted-list check, a pure parser. The
  ...
  // MochiKit scaffolder, mirroring the xpcshell scaffolder shape.
  ...
  // codebase funnels through this one module so the failure branch is
  // covered once here instead of uncovered at ~40 call sites. That only
 ...
 * and absorbs a lot, so a module can sit well below the global line threshold
 * with nothing to notice, and a new module arrives completely unguarded.
 *
 * Kept modest on purpose: it guards against rot rather than setting a target.
 * Raising a module above it is what MODULE_THRESHOLDS is for.
 */
