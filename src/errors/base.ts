 ...
 * prints a stack trace for, since the stack is the report.
 *
 ...
 * (rather than the command) was responsible. See
 * {@link GitIndexingTimeoutError} in errors/git.ts for the git-specific
 * variant.
 */
export class ExecTimeoutError extends FireForgeError {
   ...
   * a build step that hung) instead of a bare budget number. It prefers
   * stderr and falls back to stdout when stderr is silent.
   */
 ...
 * code, because the fact is the same whichever lock was contended.
 */
 ...
 * Ctrl+C, i.e. `isCancel(...)`.
 *
 * Not for a prompt the operator answered "no" to: 130 is
