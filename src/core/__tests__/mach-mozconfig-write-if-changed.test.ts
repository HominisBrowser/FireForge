...
/**
 * `engine/mozconfig` is a mach configure input, so `config.status` is
 * compared against its mtime, not its content. These tests run against a
 * real filesystem on purpose: the property under test ("an unchanged
 * render does not touch the file") is invisible to a mocked writer.
 */
