 ...
 *   (`.fireforge/lint-cache/per-patch-v1.json`) across invocations, since
...
/**
 * Lints one re-exported patch body using the hoisted context. A cache hit
 * replays the stored issues through the shared reporter and returns `null`.
 * A miss lints with the queue-wide program's pre-attributed slices (or a
 * per-patch build when the fresh diff creates files the hoisted program
 * never saw, the `--scan` adoption case) and returns the payload for
 ...
 * the body write, because the key hashes the on-disk `.patch` file that
 * this very run is about to rewrite, so a pre-write store could never
 * match a later run.
 */
export async function lintReExportedPatch(args: {
  ...
  // Tier notice, identical to runPatchLint's head. It is surfaced on both
  // the cached and fresh paths so the governing tier is never silent.
  ...
  if (checkJs) {
    ...
    if (allKnown) {
      ...
      // program has never seen, so fall back to a per-patch build with the
      // usual report scope and nothing is silently unchecked.
 ...
 * just-written `.patch` file and the refreshed queue context, which is the
 * state a later identical invocation will observe (call
