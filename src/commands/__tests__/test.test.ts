...
describe('testCommand', () => {
  ...
  it('prefers the stale-build diagnosis over the xpcshell appdir hint when distribution.sys.mjs is named', async () => {
    // Both signals match, but the stale-build path names a specific module
    // (distribution.sys.mjs) that points at a narrower root cause. The more
    // specific hint wins so the operator sees the actionable rebuild fix
    // before the broader xpcshell guidance.
    vi.mocked(testWithOutput).mockResolvedValue({
      exitCode: 1,
      stdout: '',
      stderr:
