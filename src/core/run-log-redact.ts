 ...
 *
 * Deliberately minimal and pattern-based — see {@link redactRunLogText} for
 * the exact contract. It is a seatbelt against the common accidental leak,
 * not a guarantee that a log is free of secrets.
 */
 ...
 * retained log would otherwise carry verbatim (the lead keeps the dash; the
 * key is what follows it). Empty values are left alone (nothing to hide).
 */
 ...
 *   `Token` schemes keep the scheme word; anything else masks the whole
 *   value).
 *
