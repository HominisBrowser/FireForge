 ...
 * commands, so it lives here; the declined wording is per-command and stays a
 * parameter.
 *
 * `confirmDestructive` never mutates on its own, so a caller that forgets this
 * check silently performs a "dry run" for real — hence a boolean return the
 * caller must act on rather than a helper that returns void.
