 ...
 * the missing half, and it deliberately reuses {@link probeMarionettePort},
 * which is port-generic, rather than growing a second probe.
 */
 ...
 * filenames in existence — a developer's unrelated Node service must never
...
/**
 * Refuses a mochitest dispatch when the harness's server port is held, and
 * — only for a recognized stale harness httpd, and only when the operator
 * opted in — terminates it first.
 *
 ...
 *
 * Best-effort by construction: {@link probeMarionettePort} reports an
