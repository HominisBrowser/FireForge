...
export function updateBrowserChromeTestContent(
  ...
  // One pass over the content, instead of four chained `.replace()` calls.
  //
  ...
  const alternatives: Array<{ pattern: string; to: string }> = [
    ...
    // hyphens underscored), so `panel` for `moz-panel`. Matching it loose, even
    ...
    // from boundaries. The anchored shape carries its own guards so it cannot
    ...
    // because it is embedded in generated identifiers on purpose. `-` is
    // not.
    //
  ...
  // Each alternative is its own capture group and the replacement is picked
  // by the group that matched, rather than by the matched text, which is
  // ambiguous whenever two rules can produce the same match with different
  // substitutions.
