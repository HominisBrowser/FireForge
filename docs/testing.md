...
- a changed `jar.mn` escalates the whole build to a full `mach build`
  (minutes rather than seconds), so that new install-manifest destinations
...
Whether a full build is really required for an entry added to an existing
`jar.mn` under `dist/bin` is an open question, recorded in the 0.44.0
changelog: mozbuild regenerates its backend from a newer `jar.mn` on any
`mach build`, `faster` included, and the FasterMake backend consumes
`dist/bin` jar manifests. The escalation stays until that is demonstrated
end to end against a real objdir (`build --ui` after a jar-only
registration, then a plain `test`), because relaxing a stale-artifact guard
on source reading alone trades a slow build for a silently wrong test.
